...
class SignUpForm extends Component {
  ...
  registerKakao() {
    Axios.get("http://localhost:8081/auth/kakao", {}).then((response) => {
      console.log(response.request.responseURL);
      window.location.href = response.request.responseURL;
    });
  }
  ...
  renderHeaderDuplicate(property, val) {
    if (property) return <p className="InputHeaderError">{val}</p>;
  }
      ...
      <div className="FormContainer">
        ...
        <div className="Form">
          <div className="InputHeaderContainer">
            ...
            {this.renderHeaderDuplicate(
              this.state.emailDuplicate,
              "이미 계정이 있는 이메일입니다."
            )}
          </div>
          <InputField
